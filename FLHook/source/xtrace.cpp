 ...
 *macro to symbols in your debug executable, you can omit the environment
 *variables (default).
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ** * */
...
BOOL InitSymInfo(PCSTR lpszInitialSymbolPath, BOOL bSysPath) {
    ...
    // to use the symbol handler.". SYMOPT_UNDNAME is excluded to do the
    // undecoration ourselves.
...
BOOL GetFuncInfo(ULONG fnAddress, ULONG stackAddress, LPTSTR lpszSymbol) {
    ...
    PIMAGEHLP_SYMBOL pSym =
        (PIMAGEHLP_SYMBOL)GlobalAlloc(GMEM_FIXED, dwSymSize);
    ...
    // Get symbol info
    if (SymGetSymFromAddr(GetCurrentProcess(), (ULONG)fnAddress, &dwDisp,
                          pSym)) {
        ...
        UnDecorateSymbolName(
            ...
            UNDNAME_COMPLETE | UNDNAME_NO_THISTYPE | UNDNAME_NO_SPECIAL_SYMS |
                ...
                UNDNAME_NO_ACCESS_SPECIFIERS | UNDNAME_NO_ARGUMENTS);
...
LPCTSTR GetFuncName() {
    ...
    if (bResult && callStack.AddrFrame.Offset != 0) {
        GetFuncInfo(callStack.AddrPC.Offset, callStack.AddrFrame.Offset,
                    lpszFnInfo);

        // from now on its all personal display preferences with string
        // manipulation
        ...
        // 'functionname: message' for debug builds and 'message' for release
        // builds, using the format string "%smessage" and __FUNCTION__ as an
        // argument
        strcat(lpszLast, ": ");
