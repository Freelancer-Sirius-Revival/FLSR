    ...
    **************************************************************************************************************/
    ...
    void __stdcall SPObjUpdate(struct SSPObjUpdateInfo const& ui,
        ...
        if (!(ui.vPos.x == ui.vPos.x) || !(ui.vPos.y == ui.vPos.y) ||
            ...
            !(ui.vDir.w == ui.vDir.w) || !(ui.throttle == ui.throttle)) {
...
if (set_bLogConnects)
    ...
    **************************************************************************************************************/
    ...
    void __stdcall MissionResponse(unsigned int boardIndex, unsigned long p2, bool accepted, unsigned int clientId) {

        ISERVER_LOG();
        ISERVER_LOGARG_UI(boardIndex);
        ISERVER_LOGARG_UI(p2);
        ISERVER_LOGARG_UI(accepted);
        ...
        CALL_PLUGINS_V(PLUGIN_HkIServerImpl_MissionResponse, __stdcall,
            ...
            (boardIndex, p2, accepted, clientId));

        EXECUTE_SERVER_CALL(Server.MissionResponse(boardIndex, p2, accepted, clientId));
    ...
    **************************************************************************************************************/
    ...
    void __stdcall PopUpDialog(unsigned int iClientID, unsigned int buttonClicked) {
        ISERVER_LOG();
        ISERVER_LOGARG_UI(iClientID);
        ISERVER_LOGARG_UI(buttonClicked);
