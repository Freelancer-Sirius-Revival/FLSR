...
const char *MODULE_TYPE_NICKNAMES[] = {
    "module_build",        "module_coreupgrade",
    "module_shieldgen",    "module_storage",
    "module_defense_1",    "module_m_docking",
    "module_m_jumpdrives", "module_m_hyperspace_scanner",
    "module_m_cloak",      "module_defense_2",
    "module_defense_3",    0};
...
std::wstring BuildModule::GetInfo(bool xml) {
    ...
    if (xml) {
        ...
        for (map<uint, uint>::iterator i = active_recipe.consumed_items.begin();
            ...
            if (gi) {
                info += L"<PARA/><TEXT>      - " + std::to_wstring(quantity) +
                        L"x " + HkGetWStringFromIDS(gi->iIDSName);
        ...
        for (map<uint, uint>::iterator i = active_recipe.consumed_items.begin();
            ...
            const GoodInfo *gi = GoodList::find_by_id(good);
            if (gi) {
                info += std::to_wstring(quantity) + L"x" +
                        HkGetWStringFromIDS(gi->iIDSName) + L" ";
            }
        }
    }
        ...
        for (uint i = 0; i < base->modules.size(); i++) {
            if (base->modules[i] == this) {
                ...
                case Module::TYPE_DEFENSE_1:
                    base->modules[i] =
                        new DefenseModule(base, Module::TYPE_DEFENSE_1);
                    break;
                case Module::TYPE_M_DOCKING:
                    base->modules[i] =
                        new FactoryModule(base, Module::TYPE_M_DOCKING);
                    break;
                case Module::TYPE_M_JUMPDRIVES:
                    base->modules[i] =
                        new FactoryModule(base, Module::TYPE_M_JUMPDRIVES);
                    break;
                case Module::TYPE_M_HYPERSPACE_SCANNER:
                    base->modules[i] = new FactoryModule(
                        base, Module::TYPE_M_HYPERSPACE_SCANNER);
                    break;
                case Module::TYPE_M_CLOAK:
                    base->modules[i] =
                        new FactoryModule(base, Module::TYPE_M_CLOAK);
                    break;
                case Module::TYPE_DEFENSE_2:
                    base->modules[i] =
                        new DefenseModule(base, Module::TYPE_DEFENSE_2);
                    break;
                case Module::TYPE_DEFENSE_3:
                    base->modules[i] =
                        new DefenseModule(base, Module::TYPE_DEFENSE_3);
                    break;
                default:
                    base->modules[i] = 0;
                    break;
