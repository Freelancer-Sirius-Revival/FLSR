...
void LoadSettings() {
    ...
    std::string scPluginCfgFile =
        std::string(szCurDir) + "\\flhook_plugins\\mobiledocking.cfg";
...
bool UserCmd_Process(uint client, const std::wstring &wscCmd) {
    ...
    // The allow dock command accepts a docking request.
        ...
        if (!iTargetClientID ||
            HkDistance3DByShip(iShip, iTargetShip) > 1000.0f) {
        ...
        // this command
        if (mapPendingDockingRequests.find(iTargetClientID) ==
                mapPendingDockingRequests.end() ||
            mapPendingDockingRequests[iTargetClientID].iTargetClientID !=
                client) {
            PrintUserCmdText(client,
                             L"No pending docking requests for this ship");
            return true;
        }
        ...
        // error
        if (clients[client].mapDockedShips.size() >=
            clients[client].iDockingModules) {
        ...
        std::wstring charname =
            (const wchar_t *)Players.GetActiveCharacterName(iTargetClientID);
        clients[client].mapDockedShips[charname] = charname;
        SaveDockInfo(client);

        // Save the docking ship info
        clients[iTargetClientID].mobile_docked = true;
        clients[iTargetClientID].wscDockedWithCharname =
            (const wchar_t *)Players.GetActiveCharacterName(client);
        if (clients[iTargetClientID].iLastBaseID != 0)
            clients[iTargetClientID].iLastBaseID =
                Players[iTargetClientID].iLastBaseID;
        pub::SpaceObj::GetSystem(iShip,
                                 clients[iTargetClientID].iCarrierSystem);
        pub::SpaceObj::GetLocation(iShip,
                                   clients[iTargetClientID].vCarrierLocation,
                                   clients[iTargetClientID].mCarrierLocation);
...
int __cdecl Dock_Call(unsigned int const &iShip, unsigned int const &iBaseID,
    ...
    if (client) {
        ...
        // Check that the target ship has an empty docking module. Report the
        // error
        if (clients[iTargetClientID].mapDockedShips.size() >=
            clients[iTargetClientID].iDockingModules) {
            PrintUserCmdText(client,
                             L"Target ship has no free docking capacity");
            return 0;
        }

        // Check that the requesting ship is of the appropriate size to dock.
        CShip *cship =
            (CShip *)HkGetEqObjFromObjRW((IObjRW *)HkGetInspect(client));
        ...
        PrintUserCmdText(iTargetClientID,
                         L"%s is requesting to dock, authorise with /allowdock",
                         Players.GetActiveCharacterName(client));
        PrintUserCmdText(client, L"Docking request sent to %s",
                         Players.GetActiveCharacterName(iTargetClientID));
        return -1;
    }
    return 0;
}
...
bool IsShipDockedOnCarrier(std::wstring &carrier_charname,
                           std::wstring &docked_charname) {
    uint client = HkGetClientIdFromCharname(carrier_charname);
    if (client != -1) {
        return clients[client].mapDockedShips.find(docked_charname) !=
               clients[client].mapDockedShips.end();
    } else {
        return false;
    }
...
void __stdcall BaseEnter(uint iBaseID, uint client) {
    ...
    if (clients[client].mobile_docked) {
        ...
        status += L"<TEXT>" + XMLText(clients[client].wscDockedWithCharname) +
                  L"</TEXT><PARA/><PARA/>";
        status += L"<POP/></RDL>";
        ...
        std::wstring charname =
            (const wchar_t *)Players.GetActiveCharacterName(client);
        if (!IsShipDockedOnCarrier(clients[client].wscDockedWithCharname,
                                   charname)) {
            JumpToLocation(client, clients[client].iCarrierSystem,
...
void __stdcall PlayerLaunch(unsigned int iShip, unsigned int client) {
    ...
    if (clients[client].mobile_docked) {
        ...
        // the carrier
        uint carrier_client =
            HkGetClientIdFromCharname(clients[client].wscDockedWithCharname);
        if (carrier_client != -1) {
            UpdateDockedShips(carrier_client);
            clients[carrier_client].mapDockedShips.erase(
                (const wchar_t *)Players.GetActiveCharacterName(client));
        }
...
void SystemSwitchOutComplete(unsigned int iShip, unsigned int client) {
    ...
    if (mapDeferredJumps.find(client) != mapDeferredJumps.end()) {
        ...
        *(float *)(SwitchOut + 0x2b0) =
            mapDeferredJumps[client].pos.z; // set entry location
        ...
        *(float *)(SwitchOut + 0x2c8) =
            mapDeferredJumps[client].ornt.data[2][2];
        *(float *)(SwitchOut + 0x2d0) =
            mapDeferredJumps[client].ornt.data[1][1];
        *(float *)(SwitchOut + 0x2d8) =
            mapDeferredJumps[client].ornt.data[0][0];
        *(float *)(SwitchOut + 0x2e0) =
            mapDeferredJumps[client].ornt.data[2][1];
        *(float *)(SwitchOut + 0x2e8) =
            mapDeferredJumps[client].ornt.data[2][0];
        *(float *)(SwitchOut + 0x2f0) =
            mapDeferredJumps[client].ornt.data[1][2];
        *(float *)(SwitchOut + 0x2f8) =
            mapDeferredJumps[client].ornt.data[1][0];
        *(float *)(SwitchOut + 0x300) =
            mapDeferredJumps[client].ornt.data[0][2];
        *(float *)(SwitchOut + 0x308) =
            mapDeferredJumps[client].ornt.data[0][1];
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
    ...
    p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&ReqRemoveItem_AFTER,
