...
namespace Missions
{
	...
	static CndCount* ReadCndCount(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value >= 0)
				...
				PrintErrorToConsole(ini, argNum, L"Invalid target count. Must be equal or greater than 0 Defaulting to 0.");
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "less")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid comparator. Must be LESS, GREATER, or EQUAL. Defaulting to EQUAL.");
		}
	...
	static CndDestroyed* ReadCndDestroyed(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "explode")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid destruction type. Must be EXPLODE, VANISH, or ANY. Defaulting to ANY.");
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "destroyed")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid activator target. Must be KILLER, or DESTROYED. Defaulting to INFLICTOR.");
		}
	...
	static CndDistObj* ReadCndDistObj(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		{
			PrintErrorToConsole(ini, argNum, L"No other ship. Aborting!");
			return nullptr;
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "outside")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid distance relation. Must be INSIDE, or OUTSIDE. Defaulting to INSIDE.");
		}
	...
	static CndDistVec* ReadCndDistVec(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (systemId == 0)
		{
			PrintErrorToConsole(ini, argNum, L"No system. Aborting!");
			return nullptr;
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (hardpoint.empty())
				PrintErrorToConsole(ini, argNum, L"Invalid hardpoint. Defaulting to none.");
		}
	...
	static CndHasCargo* ReadCndHasCargo(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		while (argNum < maxArgs)
		{
			...
			if (cargoId == 0)
				PrintErrorToConsole(ini, argNum, L"Invalid archetype name. Ignoring.");

			const int count = ini.get_value_int(argNum++);
			if (count <= 0)
				PrintErrorToConsole(ini, argNum, L"Invalid quantity. Defaulting to 1.");
	...
	static CndHealthDec* ReadCndHealthDec(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "damaged")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid activator target. Must be INFLICTOR, or DAMAGED. Defaulting to INFLICTOR.");
		}
		argNum++;
		...
		for (const auto maxArgs = ini.get_num_parameters(); argNum < maxArgs; argNum++)
		{
			const auto& value = CreateIdOrNull(ini.get_value_string(argNum));
			if (value == 0)
				PrintErrorToConsole(ini, argNum, L"Invalid collision group name. Ignoring.");
			else
	...
	static CndHealthInc* ReadCndHealthInc(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "repaired")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid activator target. Must be INFLICTOR, or REPAIRED. Defaulting to INFLICTOR.");
		}
		argNum++;
	...
	static CndInSystem* ReadCndInSystem(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (systemIds.empty())
		{
			PrintErrorToConsole(ini, 1, L"No system given. Aborting!");
			return nullptr;
		}
	...
	static CndInZone* ReadCndInZone(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (zoneIds.empty())
		{
			PrintErrorToConsole(ini, 1, L"No zones given. Aborting!");
			return nullptr;
		}
	...
	static CndJoinGroup* ReadCndJoinGroup(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			PrintErrorToConsole(ini, argNum, L"No target label. Defaulting to ANY.");
	...
	static CndProjHitCount* ReadCndProjHit(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value > 0)
				...
				PrintErrorToConsole(ini, argNum, L"Target hit count must be greater than 0. Defaulting to " + std::to_wstring(targetHitCount) + L".");
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "hull")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid damage surface. Must be ANY, HULL, or SHIELD. Defaulting to ANY.");
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "projectile")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid damage type. Must be ANY, PROJECTILE, or EXPLOSION. Defaulting to ANY.");
		}
		argNum++;
	...
	static CndSystemSpaceEnter* ReadCndSystemSpaceEnter(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "jump")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid enter type. Must be JUMP, LAUNCH, SPAWN, or ANY. Defaulting to ANY.");
		}
		argNum++;
	...
	static CndSystemSpaceExit* ReadCndSystemSpaceExit(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value == "jump")
				...
				PrintErrorToConsole(ini, argNum, L"Invalid enter type. Must be JUMP, DOCK, EXPLDOE, VANISH, or ANY. Defaulting to ANY.");
		}
		argNum++;
	...
	static CndTimer* ReadCndTimer(const ConditionParent& conditionParent, INI_Reader& ini)
	{
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value >= 0.0f)
				...
				PrintErrorToConsole(ini, argNum, L"Lower boundary must be equal or greater than 0. Defaulting to 0.");
		}
		argNum++;
		...
		if (ini.get_num_parameters() > argNum)
		{
			...
			if (value >= 0.0f)
				...
				PrintErrorToConsole(ini, argNum, L"Upper boundary must be equal or greater than 0. Defaulting to 0.");
		}
