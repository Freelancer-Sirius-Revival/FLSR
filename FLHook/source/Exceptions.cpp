...
HMODULE GetModuleAddr(uint iAddr) {
    ...
    if (EnumProcessModules(hProcess, hModArr, sizeof(hModArr),
                           &iArrSizeNeeded)) {
        ...
        for (uint i = 0; i < iArrSizeNeeded; i++) {
            ...
            if (GetModuleInformation(hProcess, hModArr[i], &mi, sizeof(mi))) {
                if (((uint)mi.lpBaseOfDll) < iAddr &&
                    (uint)mi.lpBaseOfDll + (uint)mi.SizeOfImage > iAddr) {
                    return hModArr[i];
                }
            }
        }
...
void WriteMiniDump(SEHException *ex) {
    ...
    if (hDll) {
        ...
        if (pDump) {
            ...
            tm t;
            localtime_s(&t, &tNow);
            strftime(szDumpPathFirst, sizeof(szDumpPathFirst),
                     "./flhook_logs/debug/flserver_%d.%m.%Y_%H.%M.%S", &t);
...
void AddExceptionInfoLog(SEHException *ex) {
    ...
    try {
        ...
        if (exception) {
            ...
            AddBothLog(std::format("Code={:x} Offset={:x} Module=\"{}\"", iCode, iOffset,
                       szModName));
            if (iCode == 0xE06D7363 &&
                exception->NumberParameters == 3) // C++ exception
            {
                const auto *info = reinterpret_cast<const msvc__ThrowInfo *>(
                    exception->ExceptionInformation[2]);
                const auto *typeArr =
                    &info->pCatchableTypeArray->arrayOfCatchableTypes;
                const auto *obj = reinterpret_cast<const std::exception *>(
                    exception->ExceptionInformation[1]);
                const char *szName =
                    info && info->pCatchableTypeArray &&
                            info->pCatchableTypeArray->nCatchableTypes
                ...
                const char *szMessage =
                    i != info->pCatchableTypeArray->nCatchableTypes
                        ? obj->what()
                        : "";
                ...
                if (reg) {
                    ...
                    if (hModExc) {
                        iOffset = iAddr - (uint)hModExc;
                        GetModuleFileName(hModExc, szModName,
                                          sizeof(szModName));
                    }
                }
                AddBothLog(std::format("Name=\"{}\" Message=\"{}\" Offset={:x} Module=\"{}\"",
                           szName, szMessage, iOffset,
                           strrchr(szModName, '\\') + 1));
            }
            ...
            for (int i = 0; i < count; i++)
                AddBothLog(std::format("{} called from {}", i, callers[i]));
        } else
            AddBothLog("No exception information available");
        if (reg) {
            AddBothLog(std::format("eax={:x} ebx={:x} ecx={:x} edx={:x} edi={:x} esi={:x} ebp={:x} eip={:x} esp={:x}",
                       reg->Eax, reg->Ebx, reg->Ecx, reg->Edx, reg->Edi,
                       reg->Esi, reg->Ebp, reg->Eip, reg->Esp));
        } else {
