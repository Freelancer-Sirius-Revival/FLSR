...
/// player.
void PrintLocalUserCmdText(uint iClientID, const std::wstring &wscMsg,
                           float fDistance) {
    uint iShip;
...
void UserCmd_IgnoreID(uint iClientID, const std::wstring &wscParam) {
    ...
    // send confirmation msg
    PrintUserCmdText(iClientID, L"OK, \"%s\" added to ignore list",
                     wscCharname.c_str());
}
...
void UserCmd_IgnoreList(uint iClientID, const std::wstring &wscParam) {
    ...
    for (auto &ignore : ClientInfo[iClientID].lstIgnore) {
        PrintUserCmdText(iClientID, L"%.2u | %s | %s", i,
                         ignore.wscCharname.c_str(), ignore.wscFlags.c_str());
        i++;
    }
...
void UserCmd_DelIgnore(uint iClientID, const std::wstring &wscParam) {
    ...
    for (auto &del : lstDelete) {
        ...
        for (auto ignoreIt = ClientInfo[iClientID].lstIgnore.begin();
             ignoreIt != ClientInfo[iClientID].lstIgnore.end(); ++ignoreIt) {
            if (iCurID == del) {
...
void UserCmd_Help(uint iClientID, const std::wstring &wscParam) {
    ...
    std::wstring boldHelp =
        set_wscUserCmdStyle.substr(0, set_wscUserCmdStyle.length() - 1) + L"1";
        ...
        if (he.fnIsDisplayed(iClientID)) {
            if (singleCommandHelp) {
                if (he.wszCommand == wscParam) {
                    ...
                    PrintUserCmdText(iClientID,
                                     he.wszCommand + L" " + he.wszArguments);
