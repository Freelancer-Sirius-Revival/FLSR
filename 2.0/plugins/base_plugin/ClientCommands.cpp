        ...
        if (!HkIsInCharSelectMenu(client)) {
            if (clients[client].player_base == base->base) {
                ...
                // Otherwise this item is for sale by the client.
                    ...
                    _snwprintf_s(
                        buf, sizeof(buf), L" SetMarketOverride %u %u %f %u %u",
...
void SendBaseStatus(uint client, PlayerBase *base) {
    ...
    base_status += L"<TEXT>" + XMLText(base->basename) + L", " +
                   HkGetWStringFromIDS(sys->strid_name) +
                   L"</TEXT><PARA/><PARA/>";

    base_status += base->infocard;

    base_status += L"<TEXT>Base Core: Level " + IntToStr(base->base_level) +
                   L"</TEXT><PARA/>";
    base_status += L"<TEXT>Free Cargo Storage: " +
                   Int64ToPrettyStr(base->GetRemainingCargoSpace()) +
                   L"</TEXT><PARA/>";
    base_status += L"<TEXT>Max Cargo Storage: " +
                   Int64ToPrettyStr(base->GetMaxCargoSpace()) +
                   L"</TEXT><PARA/>";
    base_status +=
        L"<TEXT>Money: " + Int64ToPrettyStr(base->money) + L"</TEXT><PARA/>";
    base_status += L"<TEXT>Hit Points: " +
                   Int64ToPrettyStr((INT64)base->base_health) +
                   L"</TEXT><PARA/>";
    base_status += L"<TEXT>Max Hit Points: " +
                   Int64ToPrettyStr((INT64)base->max_base_health) +
                   L"</TEXT><PARA/>";
    base_status +=
        L"<TEXT>Population: " +
        Int64ToPrettyStr((INT64)base->HasMarketItem(set_base_crew_type)) +
        L"</TEXT><PARA/>";
