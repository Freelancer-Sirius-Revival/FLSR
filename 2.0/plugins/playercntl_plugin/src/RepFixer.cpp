...
static void LoadFactionReps(const std::string &scPluginCfgFile,
                            const std::string &scIDNick) {
...
void RepFixer::LoadSettings(const std::string &scPluginCfgFile) {
    set_bEnableRepFixUpdates =
        IniGetB(scPluginCfgFile, "RepFixer", "EnableRepFixUpdates", false);
    set_bLogUpdates =
        IniGetB(scPluginCfgFile, "RepFixer", "LogRepFixUpdates", false);
    set_bItemMustBeMounted =
        IniGetB(scPluginCfgFile, "RepFixer", "ItemMustBeMounted", true);
...
static void CheckReps(unsigned int iClientID) {
    ...
    for (auto &cargo : lstCargo) {
        ...
        // if needed.
        for (auto iterReps = iterIDs->second.begin();
             iterReps != iterIDs->second.end(); iterReps++) {
            const FactionRep &rep = *iterReps;

            float fRep = 0.0f;
            HkGetRep(wscCharName, stows(rep.scRepGroup), fRep);
            if (((fRep > rep.fRep) &&
                 (rep.iMode == FactionRep::MODE_REP_LESSTHAN)) ||
                ((fRep < rep.fRep) &&
