...
namespace BatsBotsShipTransferFix
{
	...
	void __stdcall ReqEquipment(const EquipDescList& equipDescriptorList, unsigned int clientId)
	{
		...
		for (auto it = list->equip.begin(); it != list->equip.end(); )
		{
			...
			{
				case Archetype::AClassType::REPAIR_KIT:
				{
					auto result = ReduceAndRefund(it, ship->iMaxNanobots, clientId);
					refund += result.refund;
					it = result.erase ? list->equip.erase(it) : it++;
					break;
				}

				case Archetype::AClassType::SHIELD_BATTERY:
				{
					auto result = ReduceAndRefund(it, ship->iMaxShieldBats, clientId);
					refund += result.refund;
					it = result.erase ? list->equip.erase(it) : it++;
					break;
				}
