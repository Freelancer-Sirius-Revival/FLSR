...
static void EncryptDecrypt(char *ibuf, char *obuf) {
    obuf[0] = ibuf[0];
    obuf[1] = ibuf[1];
    for (uint i = 2, p = ibuf[0] % set_scEncryptKey.length(); i < HCOORD_SIZE;
         i++, p++) {
...
void HyperJump::LoadSettings(const std::string &scPluginCfgFile) {
    ...
    std::string scCfgFile =
        std::string(szCurDir) + "\\flhook_plugins\\jump.cfg";
            ...
            if (ini.is_header("general")) {
                ...
                while (ini.read_value()) {
                    if (ini.is_value("death_system")) {
                        set_death_systems[CreateID(ini.get_value_string(0))] =
                            true;
                    }
                }
        ...
        if (iShip == 0) {
            ...
            if (sm.charging_on) {
                ...
                for (auto item = Players[iClientID].equipDescList.equip.begin();
                     item != Players[iClientID].equipDescList.equip.end();
                     item++) {
                    if (sm.arch.mapFuelToUsage.find(item->iArchID) !=
                        sm.arch.mapFuelToUsage.end()) {
                        uint fuel_usage = sm.arch.mapFuelToUsage[item->iArchID];
                        if (item->iCount >= fuel_usage) {
                            pub::Player::RemoveCargo(iClientID, item->sID,
                                                     fuel_usage);
                ...
                if (sm.curr_charge >= sm.arch.survey_complete_charge) {
                    ...
                    PrintUserCmdText(iClientID, L"Coords: %s",
                                     FormatCoords(obuf).c_str());

                    lstOldClients.push_back(iClientID);
                } else if (time(0) % 10 == 0) {
                    PrintUserCmdText(
                        iClientID, L"Survey %0.0f%% complete",
                        (sm.curr_charge / sm.arch.survey_complete_charge) *
                            100.0f);
                }
            }
        }
        ...
        if (iShip == 0) {
            ...
            if (jd.jump_timer > 0) {
                ...
                if (jd.jump_timer == 7) {
                    ...
                    pub::Audio::PlaySoundEffect(
                        iClientID, CreateID("dsy_jumpdrive_activate"));
                }
                // Execute the jump and do the pop sound
                    ...
                    SwitchSystem(iClientID, jd.iTargetSystem,
                                 jd.vTargetPosition, mShipDir);
                    ...
                    if (jd.arch.field_range > 0) {
                        ...
                        while (pPD = Players.traverse_active(pPD)) {
                            ...
                            Vector vPosition2;
                            Matrix mShipDir2;
                            pub::SpaceObj::GetLocation(pPD->iShipID, vPosition2,
                                                       mShipDir2);

                            if (pPD->iOnlineID != iClientID &&
                                iSystemID2 == iSystemID &&
                                HkDistance3D(vPosition, vPosition2) <=
                                    jd.arch.field_range) {
                                PrintUserCmdText(pPD->iOnlineID, L"Jumping...");

                                Vector vNewTargetPosition;
                                vNewTargetPosition.x =
                                    jd.vTargetPosition.x +
                                    (vPosition.x - vPosition2.x);
                                vNewTargetPosition.y =
                                    jd.vTargetPosition.y +
                                    (vPosition.y - vPosition2.y);
                                vNewTargetPosition.z =
                                    jd.vTargetPosition.z +
                                    (vPosition.z - vPosition2.z);
                                pub::Audio::PlaySoundEffect(
                                    pPD->iOnlineID,
                                    CreateID("dsy_jumpdrive_activate"));
                                pub::SpaceObj::DrainShields(pPD->iShipID);
                                SwitchSystem(pPD->iOnlineID, jd.iTargetSystem,
                                             vNewTargetPosition, mShipDir2);
                            }
                        }
                    }
                }
                ...
                // (the switch out causes the ship to be invincible
                    ...
                    // kill the ship
                    if (set_death_systems.find(jd.iTargetSystem) !=
                        set_death_systems.end()) {
                        IObjInspectImpl *obj = HkGetInspect(iClientID);
                        if (obj) {
                            HkLightFuse((IObjRW *)obj, CreateID("death_comm"),
                                        0.0f, 0.0f, 0.0f);
                        }
                    }
            ...
            if (jd.charging_on) {
                ...
                for (auto item = Players[iClientID].equipDescList.equip.begin();
                     item != Players[iClientID].equipDescList.equip.end();
                     item++) {
                    if (jd.arch.mapFuelToUsage.find(item->iArchID) !=
                        jd.arch.mapFuelToUsage.end()) {
                ...
                if (!jd.charging_on) {
                    ...
                    pub::Player::SendNNMessage(
                        iClientID,
                        pub::GetNicknameId("nnv_jumpdrive_charging_failed"));
                    SetFuse(iClientID, 0);
                    StopChargeFuses(iClientID);
                    continue;
                }
                ...
                if (jd.curr_charge >= jd.arch.can_jump_charge) {
                    ...
                    if (!jd.charging_complete) {
                        PrintUserCmdText(
                            iClientID,
                            L"Jump drive charging complete, ready to jump");
                        pub::Player::SendNNMessage(
                            iClientID, pub::GetNicknameId(
                                           "nnv_jumpdrive_charging_complete"));
                ...
                uint expected_charge_status =
                    (uint)(jd.curr_charge / jd.arch.can_jump_charge * 10);
                if (jd.charge_status != expected_charge_status) {
                    jd.charge_status = expected_charge_status;
                    PrintUserCmdText(
                        iClientID, L"Jump drive charge %0.0f%%",
                        (jd.curr_charge / jd.arch.can_jump_charge) * 100.0f);

                    // Find the currently expected charge fuse
                    uint charge_fuse_idx =
                        (uint)((jd.curr_charge / jd.arch.can_jump_charge) *
                               (jd.arch.charge_fuse.size() - 1));
                    ...
                    if (find(jd.active_charge_fuse.begin(),
                             jd.active_charge_fuse.end(),
                             charge_fuse) == jd.active_charge_fuse.end()) {
    ...
    for (auto &iter : mapTestBots) {
        ...
        if (tb.bBaseTest) {
            if (tb.iCheckZonesTimer > 0) {
                ...
                if (iBase) {
                    PrintUserCmdText(iClientID, L"Launching iteration %d",
                                     tb.iCheckTestedZones);
                ...
                // if in space dock with specified base.
                    ...
                    PrintUserCmdText(iClientID, L"Docking iteration %d",
                                     tb.iCheckTestedZones);

                    const struct Universe::IBase *baseinfo =
                        Universe::get_base(tb.iCheckSystemOrBase);
                ...
                if (tb.iCheckZonesTimer > 0) {
                    if ((tb.iCheckZonesTimer % 10) == 0)
                        PrintUserCmdText(iClientID,
                                         L"Jump to next zone in %d seconds",
                                         tb.iCheckZonesTimer);
                    tb.iCheckZonesTimer--;
                } else if (tb.iCheckZonesTimer == 0) {
                    if (tb.lstCheckZones.size()) {
                        ...
                        PrintUserCmdText(
                            iClientID,
                            L"Testing zone %s (%0.0f %0.0f %0.0f) iteration %d",
                            stows(lz.zoneNick).c_str(), lz.pos.x, lz.pos.y,
                            lz.pos.z, tb.iCheckTestedZones);
...
void HyperJump::SendDeathMsg(const std::wstring &wscMsg, uint iSystem,
    ...
    if (iter != mapTestBots.end()) {
        PrintUserCmdText(iClientIDKiller,
                         L"Err 0101010001110 Does not compute");
        HkKill(
...
bool HyperJump::SystemSwitchOutComplete(unsigned int iShip,
    ...
    if (mapDeferredJumps.find(iClientID) != mapDeferredJumps.end()) {
        ...
        *(float *)(SwitchOut + 0x2b0) =
            mapDeferredJumps[iClientID].pos.z; // set entry location
        ...
        *(float *)(SwitchOut + 0x2c8) =
            mapDeferredJumps[iClientID].ornt.data[2][2];
        *(float *)(SwitchOut + 0x2d0) =
            mapDeferredJumps[iClientID].ornt.data[1][1];
        *(float *)(SwitchOut + 0x2d8) =
            mapDeferredJumps[iClientID].ornt.data[0][0];
        *(float *)(SwitchOut + 0x2e0) =
            mapDeferredJumps[iClientID].ornt.data[2][1];
        *(float *)(SwitchOut + 0x2e8) =
            mapDeferredJumps[iClientID].ornt.data[2][0];
        *(float *)(SwitchOut + 0x2f0) =
            mapDeferredJumps[iClientID].ornt.data[1][2];
        *(float *)(SwitchOut + 0x2f8) =
            mapDeferredJumps[iClientID].ornt.data[1][0];
        *(float *)(SwitchOut + 0x300) =
            mapDeferredJumps[iClientID].ornt.data[0][2];
        *(float *)(SwitchOut + 0x308) =
            mapDeferredJumps[iClientID].ornt.data[0][1];
...
void HyperJump::AdminCmd_TestBot(CCmds *cmds, const std::wstring &wscSystemNick,
    ...
    const struct Universe::ISystem *sysinfo =
        Universe::get_system(CreateID(wstos(wscSystemNick).c_str()));
    if (sysinfo) {
        if (iCheckZoneTime == 0)
            iCheckZoneTime = 60;
        ...
        for (auto i = start; i != end; i++) {
            ZONE rlz;
            if (ZoneUtilities::InDeathZone(tb.iCheckSystemOrBase, i->second.pos,
                                           rlz))
                continue;
    ...
    const struct Universe::IBase *baseinfo =
        Universe::get_base(CreateID(wstos(wscSystemNick).c_str()));
    if (baseinfo) {
        if (iCheckZoneTime == 0)
            iCheckZoneTime = 15;
...
bool InitJumpDriveInfo(uint iClientID) {
    ...
    if (mapJumpDrives.find(iClientID) == mapJumpDrives.end()) {
        ...
        for (auto item = Players[iClientID].equipDescList.equip.begin();
            ...
            if (mapJumpDriveArch.find(item->iArchID) !=
                mapJumpDriveArch.end()) {
                if (item->bMounted) {
                    mapJumpDrives[iClientID].arch =
                        mapJumpDriveArch[item->iArchID];
                    return true;
                }
            }
        }
...
bool HyperJump::UserCmd_Survey(uint iClientID, const std::wstring &wscCmd,
    ...
    if (cship->get_max_power() < sm.arch.power) {
        PrintUserCmdText(iClientID,
                         L"Insufficient power to start survey module");
        return true;
    }
...
bool HyperJump::UserCmd_SetCoords(uint iClientID, const std::wstring &wscCmd,
    ...
    std::string sbuf =
        wstos(ReplaceStr(GetParam(wscParam, L' ', 0), L"-", L""));
    if (sbuf.size() != 56) {
    ...
    for (uint i = 0, p = 0; i < HCOORD_SIZE && (p + 1) < sbuf.size();
         i++, p += 2) {
        char buf[3];
        buf[0] = sbuf[p];
        buf[1] = sbuf[p + 1];
    ...
    if (coords.time < time(0)) {
        PrintUserCmdText(
            iClientID,
            L"Warning old coordinates detected. Jump not recommended");
    ...
    const struct Universe::ISystem *sysinfo =
        Universe::get_system(jd.iTargetSystem);
    PrintUserCmdText(
        iClientID, L"OK Coordinates verified: %s %0.0f.%0.0f.%0.0f",
        HkGetWStringFromIDS(sysinfo->strid_name).c_str(),
        *(float *)&jd.vTargetPosition.x, *(float *)&jd.vTargetPosition.y,
        *(float *)&jd.vTargetPosition.z);

    int wiggle_factor = (int)coords.accuracy;
    jd.vTargetPosition.x +=
        ((rand() * 10) % wiggle_factor) - (wiggle_factor / 2);
    jd.vTargetPosition.y +=
        ((rand() * 10) % wiggle_factor) - (wiggle_factor / 2);
    jd.vTargetPosition.z +=
        ((rand() * 10) % wiggle_factor) - (wiggle_factor / 2);
...
bool HyperJump::UserCmd_ChargeJumpDrive(uint iClientID,
    ...
    if (!obj) {
        PrintUserCmdText(iClientID, L"Jump drive charging failed");
        pub::Player::SendNNMessage(
            iClientID, pub::GetNicknameId("nnv_jumpdrive_charging_failed"));
        return true;
    }
    ...
    if (jd.charging_on) {
        if (jd.iTargetSystem == 0) {
            ...
            pub::Player::SendNNMessage(
                iClientID,
                pub::GetNicknameId("nnv_jumpdrive_blind_jump_warning"));
        }

        pub::Player::SendNNMessage(
            iClientID, pub::GetNicknameId("nnv_jumpdrive_charging"));
...
bool HyperJump::UserCmd_ActivateJumpDrive(uint iClientID,
    ...
    if (!jd.charging_complete) {
        PrintUserCmdText(iClientID, L"Jump drive not ready");
        pub::Player::SendNNMessage(
            iClientID, pub::GetNicknameId("nnv_jumpdrive_not_ready"));
        return true;
    }
    ...
    if (jd.iTargetSystem == 0) {
        ...
        pub::Player::SendNNMessage(
            iClientID, pub::GetNicknameId("nnv_jumpdrive_blind_jump_warning"));
...
void HyperJump::PlayerLaunch(unsigned int iShip, unsigned int iClientID) {
    ...
    if (HkFLIniGet((const wchar_t *)Players.GetActiveCharacterName(iClientID),
                   L"tstamp", wscTimeStamp) != HKE_OK)
        return;

    FILETIME ft;
    ft.dwHighDateTime =
        strtoul(GetParam(wstos(wscTimeStamp), ',', 0).c_str(), 0, 10);
    ft.dwLowDateTime =
        strtoul(GetParam(wstos(wscTimeStamp), ',', 1).c_str(), 0, 10);
    ...
    Vector pos = {Players[iClientID].vPosition.x,
                  Players[iClientID].vPosition.y,
                  Players[iClientID].vPosition.z};
    pos.x += drift / 10;
    pos.y += drift;
    pos.z += drift / 10;
...
void HyperJump::MissileTorpHit(uint iClientID, DamageList *dmg) {
    if (mapSurvey.find(iClientID) != mapSurvey.find(iClientID)) {
        if (dmg->get_cause() == DamageCause::CruiseDisrupter || dmg->get_cause() == DamageCause::UnkDisrupter) {
            ...
            mapSurvey[iClientID].charging_on = false;
            PrintUserCmdText(iClientID,
                             L"Hyperspace survey disrupted, restart required");
        }
    }
}
} // namespace HyperJump
