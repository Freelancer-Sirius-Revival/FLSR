...
IMPORT float HkDistance3D(const Vector& v1, const Vector& v2);
IMPORT float HkDistance3DByShip(uint iShip1, uint iShip2);
IMPORT Quaternion HkMatrixToQuaternion(const Matrix& m);
...
IMPORT HK_ERROR HkAddEquip(const std::wstring& wscCharname, uint iGoodID, const std::string& scHardpoint, bool bMounted);
