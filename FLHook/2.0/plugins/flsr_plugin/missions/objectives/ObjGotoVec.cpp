...
namespace Missions
{
	...
	void ObjGotoVec::Execute(const uint objId) const
	{
		...
		const auto& condition = ConditionPtr(new ObjCndDistVec(parent, objectiveIndex, objId, range, position));
		condition->Register();
		missions.at(parent.missionId).dynamicConditions.insert({ condition.get(), condition });
	}
}