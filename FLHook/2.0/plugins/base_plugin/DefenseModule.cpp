...
static uint CreateWPlatformNPC(uint iSystem, Vector position, Matrix rotation,
                               uint solar_ids, uint type) {
...
static uint CreateWPlatformSolar(PlayerBase *base, uint iSystem,
                                 Vector position, Matrix rotation,
                                 uint solar_ids, uint type) {
    ...
    strncpy_s(si.cNickName, sizeof(si.cNickName), wplatform_nickname.c_str(),
              wplatform_nickname.size());
...
bool DefenseModule::Timer(uint time) {
    ...
    if (!space_obj) {
        if (set_new_spawn)
            space_obj =
                CreateWPlatformSolar(base, base->system, pos, EulerMatrix(rot),
                                     base->solar_ids, type);
        else
            space_obj = CreateWPlatformNPC(base->system, pos, EulerMatrix(rot),
                                           base->solar_ids, type);
...
float DefenseModule::SpaceObjDamaged(uint space_obj, uint attacking_space_obj,
                                     float curr_hitpoints, float damage) {
    base->SpaceObjDamaged(space_obj, attacking_space_obj, curr_hitpoints,
                          damage);
    return damage;
}
