    ...
    try {
        ...
        if (hKernel32) {
            ...
            if (dwOrgEntry) {
                ...
                ReadProcMem((char *)hKernel32 + offset,
                            oldSetUnhandledExceptionFilter, 5);
...
void FLHookShutdown() {
    ...
    if (hKernel32) {
        ...
        if (dwOrgEntry) {
            DWORD offset = (char *)dwOrgEntry - (char *)hKernel32;
            WriteProcMem((char *)hKernel32 + offset,
                         oldSetUnhandledExceptionFilter, 5);
        }
    }
...
void ProcessPendingCommands() {
    TRY_HOOK {
        ...
        if (sListen !=
            INVALID_SOCKET) { // check for new ascii socket connections
            FD_SET fds;
            FD_ZERO(&fds);
            FD_SET(sListen, &fds);
